...
class Member{	//접근제어자 private(캡슐화)를 넣는 순간, 멤버 안에서만 사용이 가능.  
	...
	//this 1.지역변수와 멤버변수가 총돌날 때 this를 붙이면멤버변수를 의미한다. 
	//	   2. 생성자를 찾을 때.. (
	public Member(String name, int age) { //생성자 Member를 s1, s2에 
		// 생성자는 디폴트다. 만들지 않아도 자바가 메모리 내에 알아서 생성
		this.name = name; 	//this가 없는 name은 기존 메모리 공간에 있던 private String name을 의미하고
							//this.name은 생성자 Member(name 안에 있는 heap 내의 name(속성) 의미한다.
		this.age  = age;
	}
	public Member() {
		this("미정의",1);		//생성자를 찾는데, Member와 같은 생성자를 찾는다.
							//"미정의" 문자, 1 정수. (문자,정수)에 맞는 생성자를 찾아서 대입
	}
